...
fn integration_storage_test() {
    ...
    let storage_entrypoint_selector =
        Felt252::from_bytes_be(&calculate_sn_keccak("write_and_read".as_bytes()));
...
fn test_storage_struct() {
    ...
    let storage_entrypoint_selector =
        Felt252::from_bytes_be(&calculate_sn_keccak("write_at".as_bytes()));
    ...
    let first_key = calculate_sn_keccak("_struct".as_bytes());
    let mut second_key = first_key.clone();
    second_key[31] += 1;

    let (n_modified_contracts, n_storage_updates) = state.count_actual_storage_changes();

    assert!(!state.cache().storage_writes().is_empty());
    dbg!(first_key.clone());
    dbg!(state.cache().storage_writes());
    assert_eq!(
        state
            .cache()
            .storage_writes()
            .get(&(address.clone(), first_key))
            .cloned(),
        Some(Felt252::new(555))
    );
    assert_eq!(
        state
            .cache()
            .storage_writes()
            .get(&(address, second_key))
            .cloned(),
        Some(Felt252::new(555))
    );
    assert_eq!(n_modified_contracts, 1);
    assert_eq!(n_storage_updates, 1);
}
