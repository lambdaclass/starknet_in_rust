...
lazy_static! {
    static ref TEST_ACCOUNT_COMPILED_CONTRACT_CLASS_HASH: ClassHash = ClassHash::from([1; 32]);
}
...
fn internal_deploy_account() {
    ...
    let class_hash = ClassHash::from(compute_deprecated_class_hash(&contract_class).unwrap());
...
fn internal_deploy_account_cairo1() {
    ...
    let internal_deploy_account = DeployAccount::new(
        ClassHash::from(*TEST_ACCOUNT_COMPILED_CONTRACT_CLASS_HASH),
        0,
        1.into(),
        Felt252::zero(),
        vec![2.into()],
    ...
    let accessed_keys: ClassHash = ClassHash::from([
    ...
    assert_eq!(
        ...
        TransactionExecutionInfo::new(
            Some(CallInfo {
                ...
                #[cfg(feature="cairo_1_tests")]
                gas_consumed: 16770,
                class_hash: Some(ClassHash::from([1;32])),
