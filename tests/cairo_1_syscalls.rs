...
use starknet_in_rust::{utils::ClassHash, EntryPointType};
...
fn storage_write_read() {
    ...
    let class_hash: ClassHash = ClassHash::from(ClassHash::from([1; 32]));
...
fn library_call() {
    ...
    let lib_class_hash: ClassHash = ClassHash::from(ClassHash::from([2; 32]));
...
fn call_contract_storage_write_read() {
    ...
    let simple_wallet_class_hash: ClassHash = ClassHash::from(ClassHash::from([2; 32]));
...
fn deploy_cairo1_from_cairo1() {
    // data to deploy
    let test_class_hash: ClassHash = ClassHash::from(ClassHash::from([2; 32]));
...
fn replace_class_internal() {
    ...
    let address = Address(1111.into());
    let class_hash_a: ClassHash = ClassHash::from(ClassHash::from([1; 32]));
    let nonce = Felt252::zero();
    ...
    let class_hash_b: ClassHash = ClassHash::from([2; 32]);
    ...
    // Check that the class_hash_b.to_bytes_be() leads to contract_class_b for soundness
    assert_eq!(
...
fn replace_class_contract_call() {
    ...
    */
    ...
    let class_hash_a: ClassHash = ClassHash::from([1; 32]);
    let nonce = Felt252::zero();
    ...
    let wrapper_class_hash: ClassHash = ClassHash::from([3; 32]);
...
fn call_contract_downgrade_cairo_1_to_cairo_0_same_transaction() {
    ...
    */
    ...
    let calldata = [Felt252::from_bytes_be(&class_hash_c.to_bytes_be())].to_vec();
    let caller_address = Address(0000.into());
    let entry_point_type = EntryPointType::External;
...
fn library_call_failure() {
    ...
    let lib_address = Address(1112.into());
    let lib_class_hash: ClassHash = ClassHash::from([2; 32]);
    let lib_nonce = Felt252::zero();
...
fn send_messages_to_l1_different_contract_calls() {
    ...
    let send_msg_class_hash: ClassHash = ClassHash::from([2; 32]);
