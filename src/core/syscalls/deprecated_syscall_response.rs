use super::deprecated_syscall_request::{
    CountFields, DeprecatedCallContractRequest, DeprecatedDeployRequestStruct,
...
impl DeprecatedWriteSyscallResponse for DeprecatedDeployResponse {
    fn write_syscall_response(
        ...
        vm.insert_value(
            (syscall_ptr + DeprecatedDeployRequestStruct::count_fields())?,
            self.contract_address.clone(),
        )?;
        vm.insert_value(
            (syscall_ptr + (DeprecatedDeployRequestStruct::count_fields() + 1))?,
            self.constructor_retdata_size.clone(),
        )?;
        vm.insert_value(
            (syscall_ptr + (DeprecatedDeployRequestStruct::count_fields() + 2))?,
