...
pub fn calculate_contract_address(
    ...
    let contract_address_prefix = Felt252::from_bytes_be("STARKNET_CONTRACT_ADDRESS".as_bytes()).unwrap();
...
pub fn compute_hash_on_elements(vec: &[Felt252]) -> Result<Felt252, HashError> {
    ...
    let result = Felt252::from_bytes_be(&felt_result.to_bytes_be()).unwrap();
    Ok(result)
}
