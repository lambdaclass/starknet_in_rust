...
use {
    ...
    cairo_native::{cache::ProgramCache, metadata::syscall_handler::SyscallHandlerMeta},
...
impl ExecutionEntryPoint {
        ...
        C: ContractClassCache,
    {
        ...
        let contract_class = state
            ...
            .ok()
            .and_then(|compiled_class_hash| state.get_contract_class(&compiled_class_hash).ok())
    ...
    fn _execute_version0_class<S: StateReader, C: ContractClassCache>(
        ...
        let entry_point =
            self.get_selected_entry_point_v0(&contract_class, ClassHash(class_hash))?;
