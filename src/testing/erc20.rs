...
use std::{collections::HashMap, vec, io::Bytes, path::Path};
...
use crate::{
    ...
    }, Felt252, transaction::{InvokeFunction, DeployAccount, error::TransactionError}, execution::{TransactionExecutionContext, execution_entry_point::ExecutionEntryPoint, CallType}, utils::calculate_sn_keccak, services::api::contract_classes::deprecated_contract_class::ContractClass,
};
...
fn deploy_erc20_cairo2_without_constructor() {
        ...
        let calldata = vec![Felt252::from_bytes_be(&erc20_class_hash), erc20_salt.clone(), recipient.clone(), name_.clone(), decimals_.clone(), initial_supply.clone(), symbol_.clone()];
        ...
        println!("deploy erc20 call_info: {:?}", call_info);
        println!("message: {:?}", std::str::from_utf8(&Felt252::to_be_bytes(&erc20_address)));
    ...
    println!("erc20 contract address: {:?}", erc20_address);
    println!("message: {:?}", std::str::from_utf8(&Felt252::to_be_bytes(&erc20_address)));
